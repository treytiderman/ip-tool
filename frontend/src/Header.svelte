...
<header class="flex center-y gap-1">
    ...
    {#if title == "IPv4 Presets"}
        <button class="transparent" on:click={() => setPage("Settings")} title="Go to Settings page">
    ...
    <div class="grow" style="--wails-draggable:drag; cursor: pointer; user-select: none; white-space: nowrap;">
        {title}
    </div>

    {#if $settingsStore.windowAlwaysOnTop}
        <button
            class="transparent"
            on:click={() => ($settingsStore.windowAlwaysOnTop = !$settingsStore.windowAlwaysOnTop)}
    ...
    {/if}
    <button class="transparent" on:click={windowSetMinimise} title="Minimise App">
        <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"

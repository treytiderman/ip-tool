...
<!-- <div class="grid gap-xs pad-sm" style="padding-top: 1px;"> -->
    <div class="flex bottom gap-xs pad-inline-sm">
        <label>Interface</label>
        <button class="ip-icon-button color-dim" style="margin-left: auto;" title="Change Interface" on:click={() => setPage("Interfaces")} >
            <svg
    ...
    <section class="radius shadow" style="margin-inline: var(--gap-sm); background-color: var(--color-bg-1);">
        <div class="flex center-y gap-sm pad-sm">
                ...
                {/if}
            <div class="grow">{$nics[$currentNicIndex].interface_name}</div>
            <small class="color-dim">[{$nics[$currentNicIndex].interface_metric}]</small>
        </div>

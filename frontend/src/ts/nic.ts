...
function setNic(name: string) {
    ...
    if (foundIndex === -1) {
        console.log(
            "ui: Network Interface (nic) Selected does not exist",
            name,
        );
        return;
    }
    ...
    console.log(
        "ui: Network Interface (nic) Selected",
        tempNics,
        tempNics[foundIndex],
    );
...
function resetNic() {
    ...
    if (tempNics.length > 0) {
        console.log(
            "ui: Network Interface Resetting:",
            lastSelectedWaitCounter,
            tempNics[0].interface_name,
        );
...
async function updateNics() {
    ...
    const foundIndex = tempNics.findIndex(
        (nic) => nic.interface_name === tempLastSelectedNicName,
    );
    if (foundIndex === -1) {
        console.log(
            "api: Network Interfaces (nics) no longer found",
            tempLastSelectedNicName || undefined,
        );
        resetNic();
        return;
    }
